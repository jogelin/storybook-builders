...
export function getAngularCliWebpackConfigOptions(dirToSearch) {
  ...
  const normalizedAssets = normalizeAssetPatterns(projectOptions.assets, dirToSearch, project.sourceRoot);
...
export function applyAngularCliWebpackConfig(baseConfig, cliWebpackConfigOptions) {
  ...
  const rulesExcludingStyles = baseConfig.module.rules.filter(
    rule => !rule.test || (rule.test.toString() !== '/\\.css$/' && rule.test.toString() !== '/\\.scss$/')
  );
  ...
  const entry = {
    ...
    iframe: []
      ...
      .concat(Object.values(cliStyleConfig.entry).reduce((acc: any[], item) => acc.concat(item), []))
  };
